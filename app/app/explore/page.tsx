...
export default async function ExplorePage() { await requireAuth("/app/explore"); const profile = await getCurrentProfile(); if (!profile) redirect("/access-denied?reason=profile&next=%2Fapp%2Fexplore"); const courses = await getLmsCatalog(); return <UnifiedAppShell profile={profile}><div className="app-page unified-explore"><AppBreadcrumb items={[{ label: "Accueil", href: "/app" }, { label: "Explorer" }]} /><AppPageHeader eyebrow="Explorer" title="Parcours publiés" description="Le catalogue reste volontairement limité aux parcours publiés et publics." /><div className="unified-explore-grid">{courses.map((course) => <article key={course.id}><span>{course.domain.name}</span><h2>{course.title}</h2><p>{course.description}</p><Link className="btn btn-secondary" href={`/formations/${course.slug}`}><Compass size={16} aria-hidden="true" /> Consulter <ArrowRight size={16} aria-hidden="true" /></Link></article>)}</div></div></UnifiedAppShell>; }