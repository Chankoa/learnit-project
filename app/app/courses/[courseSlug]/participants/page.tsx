...
export default async function CourseParticipantsPage({ params }: { params: Promise<{ courseSlug: string }> }) {
  ...
  if (!course) notFound();
  return <UnifiedAppShell profile={profile}><div className="app-page unified-course-overview"><AppBreadcrumb items={[{ label: "Mes parcours", href: "/app/courses" }, { label: course.title, href: `/app/courses/${courseSlug}?mode=view` }, { label: "Participants" }]} /><AppPageHeader eyebrow="Parcours / Participants" title={course.title} description={`${course.enrolledLearnerCount ?? 0} inscription(s) à ce parcours.`} /><CourseContextNavigation courseSlug={courseSlug} active="participants" canPublish={context.canPublish} canManageMembers={context.canManageMembers} /><section className="course-participants-panel" aria-labelledby="participants-title"><h2 id="participants-title">Participants</h2><p>Les inscriptions sont gérées depuis les données réelles du parcours. Les invitations et les discussions collaboratives sont à venir.</p><Link className="btn btn-secondary" href={`/app/courses/${courseSlug}?mode=view`}>Vue d’ensemble</Link></section></div></UnifiedAppShell>;
}
